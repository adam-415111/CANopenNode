...
CO_ReturnError_t CO_CANsend(CO_CANmodule_t *CANmodule, CO_CANtx_t *buffer){
	...
	std::string msg = "!0 0 " + boost::lexical_cast<std::string>(buffer->ident) + " " + boost::lexical_cast<std::string>((int)buffer->DLC);
...
/******************************************************************************/
void CO_CANinterrupt_Rx(const CO_CANrxMsg_t *rcvMsg)
{
  #ifdef CO_DEBUG
  //printf("RECEIVING: ");
  #endif
  ...
  if(_CANmodule->useCANrxFilters) {
    ...
    if(index < _CANmodule->rxSize) {
      ...
      for(index = _CANmodule->rxSize; index > 0U; index--) {
	    //std::cout << (rcvMsgIdent & buffer->mask) << std::endl;
	    //std::cout << (buffer->ident & buffer->mask) << std::endl;
          if(((rcvMsgIdent ^ buffer->ident) & buffer->mask) == 0U){
              msgMatched = true;
              break;
          }
